...
public class Gameplay {
    ...
    public static void main(String[] args) {
        ...
        Round.round_result(player_hand.getCard(0).getPoints()+player_hand.getCard(1).getPoints(), dealer_hand.getCard(0).getPoints(), false, round_state);
        Round.choice(player_hand, dealer_hand, final_deck, round_state);
    }

    public static void start_game(Round state) {
        Deck final_deck = new Deck();
        final_deck.createDeck();
        Hand player_hand = new Hand();
        player_hand.initializeHand();
        ...
        dealer_hand.withdraw(final_deck);
        state.round_count++;
        System.out.println("Раунд "+state.round_count+"\nДилер раздал карты\n\tВаши карты: ["+player_hand.getCard(0).getName()+" ("+player_hand.getCard(0).getPoints()+"), "+player_hand.getCard(1).getName()+" (" + player_hand.getCard(1).getPoints() + ")] -> "+(player_hand.getCard(0).getPoints()+player_hand.getCard(1).getPoints()+"\n\tКарты дилера: ["+dealer_hand.getCard(0).getName()+" ("+dealer_hand.getCard(0).getPoints()+"), <закрытая карта>]"));
        Round.round_result(player_hand.getCard(0).getPoints()+player_hand.getCard(1).getPoints(), dealer_hand.getCard(0).getPoints(), false, state);
        Round.choice(player_hand, dealer_hand, final_deck, state);
    }
}
